...
class GeoTiff:
    def __init__(
        self, input: Union[str, np.ndarray], name: str = None, meta: dict = None
    ) -> None:
    ...
    def get_image_for_show(self, norm_pattern=None):

        image = []
        for idx, band in enumerate(self.__bands):
            if norm_pattern != None:
                band = normalize(band, norm_pattern[idx])
                image.append(band)
            else:
        ...
        image = np.transpose(
            image, (1, 2, 0)
        )  # * (channel, height, width) -> (heigth, width, channel)

        return image

    def save_image(self, output_path: str, norm_pattern=None, dpi=1000):

        image = self.get_image_for_show(norm_pattern=norm_pattern)
        plt.imshow(image)
        plt.axis("off")
        plt.savefig(output_path, dpi=dpi)


def get_image_for_show(bands: np.ndarray, norm_pattern=None):
...
def geotiff_cropping(
    ...
    if output_folder.is_file():
        raise ValueError("the output_folder must be a folder.")
    ...
    for feature in features:
        ...
        if output_type == OutputType.NumPy:
            img, out_transform = rasterio.mask.mask(
                input_tiff.reader, shapes, crop=True, nodata=no_data
            )
            ...
            img, out_transform = rasterio.mask.mask(
                input_tiff.reader, shapes, crop=True
            )
            output_meta.update(
                {
                    "driver": "GTiff",
